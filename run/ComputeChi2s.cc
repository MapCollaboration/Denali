...
void compute_chi2s(std::string ResultFolder, int member_index, std::string LHAPDFSet, std::string ResultName)
{
  ...
  NangaParbat::Parameterisation *LHAPDF_FFs = new Denali::LHAPDFparameterisation(LHAPDFSet, g, member_index);

  // Initialiase chi2 object
  NangaParbat::ChiSquare *chi2 = new Denali::AnalyticChiSquare{LHAPDF_FFs};
  ...
  for (auto const& ds : config["Data"]["sets"])
    {
      ...
      // Predictions
      NangaParbat::ConvolutionTable *PH = new Denali::PredictionsHandler{config["Predictions"], *DH, g, cuts};
