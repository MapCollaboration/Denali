...
//          Amedeo Chiefa: amedeo.chiefa@edu.unito.it
  ...
  for (auto const& ds : config["Data"]["sets"])
    {
      ...
      DSVect.push_back(std::make_pair(DH, new Denali::PredictionsHandler{config["Predictions"], *DH, g}));
    }
  ...
  for (int iexp = 0; iexp < (int) DSVect.size(); iexp++)
    {
          ...
          for (int irep = 1; irep <= nrep; irep++)
            {
              ...
              Denali::AnalyticChiSquare chi2{DSVect, new Denali::LHAPDFparameterisation{sets, g, irep}};
      ...
      // uncertainty.
      else
        {
          ...
          Denali::AnalyticChiSquare chi2{DSVect, new Denali::LHAPDFparameterisation{sets, g, 0}};
          const std::vector<double> prds = DSVect[iexp].second->GetPredictions([](double const &, double const &, double const &) -> double { return 0; });
          const std::pair<std::vector<double>, double> shifts = chi2.GetSystematicShifts(iexp);
          for (int i = 0; i < (int) bins.size(); i++)
            {
